...
Checkout.prototype = {
    ...
    setLoadWaiting: function(step, keepDisabled) {
        if (step) {
            ...
            if(step == 'shipping')
            {Element.show('billing-please-wait');}    
            else{Element.show(step+'-please-wait');}
            
        } else {
            if (this.loadWaiting) {
                
                if(!(($('billing:use_for_shipping_no')) && ($('billing:use_for_shipping_no').checked) && this.loadWaiting =='billing'))
                {
                    if(this.loadWaiting =='shipping')
                        this.loadWaiting = 'billing';    
    ...
    { console.log('inside goto section');
        var sectionElement = $('opc-'+section); console.log(sectionElement);
    ...
    setStepResponse: function(response){
        ...
        if (response.allow_sections) {
            response.allow_sections.each(function(e){
                if($('opc-'+response.goto_section))
                {$('opc-'+response.goto_section).addClassName('allow');}
            });
        }
        ...
        if (response.goto_section) { console.log('go to section called');
...
ShippingMethod.prototype = {
    ...
    nextStep: function(transport){
        ...
        if (response.goto_section) {
            ...
            checkout.reloadProgressBlock(response.goto_section);
            return;
        }
    ...
    init : function () {
        ...
        for (var i=0; i<elements.length; i++) {
            if (elements[i].name=='payment[method]') {
                ...
                if(elements[i].readAttribute('name') != 'form_key')
...
Review.prototype = {
    ...
    save: function(){
        ...
        $('affiliate_partner_name').disabled = false;
        params += '&'+$('affiliate-partner-name-form').serialize();        
        params.save = true;
        var request = new Ajax.Request(
            this.saveUrl,
            {
